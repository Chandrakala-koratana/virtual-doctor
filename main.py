...
workout = pd.read_csv("dataset/workout_dff1.csv")
description = pd.read_csv("dataset/description.csv")
medications = pd.read_csv('dataset/medicationss.csv')
diets = pd.read_csv("dataset/dietss.csv")

# Helper function to get details related to disease
def helper(dis):
...
# Symptoms dictionary to map symptoms to index values
symptoms_dict = {
...
# Diseases dictionary to map predicted value to disease name
diseases_list = {
...
@app.route("/index")
def index():
    return render_template("index2.html", symptoms_dict=symptoms_dict)
...
def home():
    if request.method == 'POST':
        ...
        symptoms_text = request.form.get('symptoms')  # Get text input
        if not symptoms_text:
            message = "Please enter at least one symptom."
            return render_template('index2.html', message=message, symptoms_dict=symptoms_dict)
        else:
            selected_symptoms = [s.strip() for s in symptoms_text.split(',')]  # Convert to list
            selected_symptoms = [s for s in selected_symptoms if s in symptoms_dict]  # Filter valid symptoms
            
            if not selected_symptoms:
                message = "Invalid symptoms entered. Please check your input."
                return render_template('index2.html', message=message, symptoms_dict=symptoms_dict)
            ...
            return render_template('index2.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                   ...
                                   my_precautions=my_precautions, medications=medications, my_diet=rec_diet, workout=workout, symptoms_dict=symptoms_dict)
    return render_template("index2.html", symptoms_dict=symptoms_dict)
